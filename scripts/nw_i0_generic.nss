    ...
    if (iHaveMaster) {
      if (GetMaster(oIntruder) == oMaster) {
          if (GetMaster(oClosest) == oMaster)
              ...
              oIntruder = oClosest;
      } else if (GetMaster(oClosest) == GetMaster())
              oClosest = oIntruder;
    }
       ...
       (GetIsObjectValid(oIntruder) && GetIsObjectValid(GetMaster())))
    {
        ...
        if (iHaveMaster &&
           ...
           if (iAmHenchman) {
             ...
             if (d100() > 95) {
                if (fChallenge < -3.) SpeakString("Don't make me laugh!");
                else if (fChallenge < -1.) SpeakString("We'll best them yet!");
                else if (fChallenge < 2.) SpeakString("Watch out for this one!");
                else SpeakString("Gods help us!");
             }
...
void RespondToShout(object oShouter, int nShoutIndex, object oIntruder = OBJECT_INVALID)
{
    ...
    switch (nShoutIndex)
    {
        ...
        case ASSOCIATE_COMMAND_ATTACKNEAREST: //Used to de-activate AGGRESSIVE DEFEND MODE
            {
              object oClosest =  GetNearestCreature(CREATURE_TYPE_REPUTATION,
              ...
              if (!GetIsObjectValid(oClosest) &&
                  ...
                  !GetIsObjectValid(GetLastHostileActor(GetMaster()))) {
                       ...
                       GetIsInLineOfSight(oLastObject))
                    {
                        ...
                        else if(GetAbilityScore(OBJECT_SELF, ABILITY_STRENGTH) >= 16 && !iAmMage)
                        {
                            if(GetIsDoorActionPossible(oLastObject, DOOR_ACTION_BASH) || GetIsPlaceableObjectActionPossible(oLastObject, PLACEABLE_ACTION_BASH))
                            {
                                 ClearAllActions();
                                 PlayVoiceChat(VOICE_CHAT_CANDO);
                                 SpeakString("I can't pick it, so I'll bash it!");
        ...
        case ASSOCIATE_COMMAND_FOLLOWMASTER: //Only used to retreat, or break free from Stand Ground Mode
            {
                if (!GetAssociateState(NW_ASC_MODE_STAND_GROUND)) {
                    if (GetAssociate(ASSOCIATE_TYPE_HENCHMAN,GetMaster()) == OBJECT_SELF)
                        SpeakString("I will follow but not attack our enemies until you tell me otherwise.");
                    else if (GetAssociate(ASSOCIATE_TYPE_FAMILIAR,GetMaster()) == OBJECT_SELF)
                        SpeakString("I will be happy to follow you, avoiding combat until you tell me otherwise!");
                    else if (GetAssociate(ASSOCIATE_TYPE_ANIMALCOMPANION,GetMaster()) == OBJECT_SELF)
                        SpeakString("<"+GetName(OBJECT_SELF)+" understands that it should follow, waiting for your command to attack.>");
                    else
                        SpeakString("<The "+GetName(OBJECT_SELF)+" will now follow you, and be peaceful until told otherwise.>");
        ...
        if(GetLocked(oLastObject) ||
           (GetIsTrapped(oLastObject) &&
            (GetTrapDetectedBy(oLastObject,OBJECT_SELF) ||
             GetTrapDetectedBy(oLastObject,GetMaster()))))
        {
            return oLastObject;
        }
        nCnt++;
...
void MyPrintString(string sString)
{
//    if (GetHenchman(GetMaster()) == OBJECT_SELF)
//        SpeakString(sString);
}
