    //  * Tag Declaration
    //  * Tag Declaration
    string sOpalTag = "SR_HEN_OPA";
    string sArgTag = "SR_HEN_ARG";
    string sTissyTag  =  "SR_HEN_TIS" ;
    string sZesdinTag =  "SR_HEN_ZES" ;
    string sEdearaTag    =   "SR_HEN_EDE";
    string sNalconTag    =  "SR_HEN_NAL";
    string sBrunaTag    =  "SR_HEN_BRU";
    string sMardrinTag    =  "SR_HEN_MAR";
    string sLiriwynTag    =  "SR_HEN_LIR";

    // * Integers
    int INT_NUM_HENCHMEN = 9;
...
void DestroyAllPersonalItems(object oPC)
{
    ...
    for (i=1; i<=INT_NUM_HENCHMEN; i++)
    {
        switch (i)
        {
            case 1: sItem = sOpalTag; break;
            case 2: sItem = sArgTag; break;
            case 3: sItem = sTissyTag; break;
            case 4: sItem = sZesdinTag; break;
            case 5: sItem = sEdearaTag; break;
            case 6: sItem = sNalconTag; break;
            case 7: sItem = sBrunaTag; break;
            case 8: sItem = sMardrinTag; break;
            case 9: sItem = sLiriwynTag; break;
        }
...
int GetHasSimilarItem(object oHench,object oTest)
{
   ...
   while (GetIsObjectValid(oItem)) {
                ...
                if (GetStringLength(s1) == GetStringLength(s2)) {
                    ...
                    if (GetStringLeft(s1,iLength) == GetStringLeft(s2,iLength)) {
                        if (is1 > 0 && is1 > StringToInt(GetStringRight(s2,1))) {
                            if (GetIdentified(oTest)) SetIdentified(oItem,TRUE);                        PrintString(GetName(oTest)+" is similar to "+GetName(oItem));
                            PrintString(GetName(oTest)+" is similar to "+GetName(oItem));
                            return TRUE;
                        } else {
                            DestroyObject(oItem);
...
{
   int  i;
   int nMasterLevel = GetHitDice(MeLevel);
   int nLevel =  nMasterLevel + 1;
   object oItem, oCopy;
...
//   SpeakString(sNewFile);
   ...
   for(i = 0; i < NUM_INVENTORY_SLOTS; ++i) {
        ...
        if (oItem != OBJECT_INVALID && oItem != GetLocalObject(MeLevel,"Decency"))
           switch (GetBaseItemType(oItem)) {
            case BASE_ITEM_CREATUREITEM: break;
